<template>
  <div>
    ...
    >
      <div
        class="flex flex-col md:flex-row items-start md:items-center space-x-0 md:space-x-6 min-w-max"
      >
        <div
          class="w-16 h-16 md:w-[88px] md:h-[88px] relative -top-8 md:top-0 rounded-full overflow-hidden"
        >
        ...
        <div class="space-y-2">
          <div class="space-x-1">
            ...
            <span
              v-if="job.isFeatured"
              class="bg-grayish-cyan-dark text-white text-xs uppercase p-2 pb-1.5 rounded-full"
              >Featured</span
            >
            <span
              v-if="job.isNew"
              class="bg-cyan-dark text-white text-xs uppercase p-2 pb-1.5 rounded-full"
              >New</span
            >
          </div>
...
import getRandomHexCode from '@/helpers/getRandomHexCode'
...
const ImageUrl = computed(() => {
  return `http://dummyimage.com/150x150.png/${getRandomHexCode()}/ffffff}`
})
</script>
